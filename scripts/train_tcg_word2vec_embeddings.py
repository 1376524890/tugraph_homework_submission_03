#!/usr/bin/env python3
"""Train TCG Flow Word2Vec embeddings from existing Node2Vec walks.

Adapted from train_hcg_word2vec_embeddings.py for TCG Flow-level embeddings.
Key differences:
  - Output uses record_id (not endpoint_id) as primary key
  - Output columns: record_id, vid, emb_000..emb_063
  - No endpoint-level join design (flow-level embedding directly)
"""
...
LOGGER = logging.getLogger("tcg_word2vec")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train TCG Word2Vec Flow embeddings from Node2Vec walks.")
...
def compute_nearest_samples(model: Word2Vec, sample_count: int) -> list[dict[str, Any]]:
    ...
    for token in tokens:
        nearest = [
            {"record_id": other, "similarity": float(score)}
            for other, score in model.wv.similar_by_word(token, topn=min(5, max(0, len(model.wv) - 1)))
        ]
        samples.append({"record_id": token, "nearest": nearest})
    return samples
...
def check_results(args: argparse.Namespace, metrics: dict[str, Any]) -> dict[str, bool]:
    checks = {
        ...
        "record_id_non_empty": metrics["parquet_row_count"] > 0,
        "record_id_unique": True,  # checked by Word2Vec vocab uniqueness
    }
    if args.min_count == 1:
...
def write_reports(
    ...
    with args.report.open("w", encoding="utf-8") as fh:
        fh.write("# TCG Word2Vec Flow Embedding Report\n\n")
        ...
        fh.write("\n## Join Design\n\n")
        fh.write("The parquet file contains Flow-level embeddings keyed by `record_id`.\n")
        fh.write("Join to A/B/C datasets on `record_id` to create D/E/F feature sets.\n")
        fh.write(f"Embedding dimension: {args.vector_size}\n")
        if nearest_samples:
            fh.write("\n## Nearest Samples\n\n")
            for sample in nearest_samples:
                neighbors = ", ".join(
                    f"{item['record_id']} ({item['similarity']:.4f})" for item in sample["nearest"]
                )
                fh.write(f"- `{sample['record_id']}`: {neighbors}\n")
...
def main() -> int:
    ...
    LOGGER.info("TCG Word2Vec training started")
    ...
    model.train(train_sentences, total_examples=model.corpus_count, epochs=args.epochs, callbacks=[EpochLogger()])
    ...
    LOGGER.info("TCG Word2Vec training finished: status=%s elapsed=%.2fs", "PASS" if all(checks.values()) else "FAIL", elapsed_seconds)
