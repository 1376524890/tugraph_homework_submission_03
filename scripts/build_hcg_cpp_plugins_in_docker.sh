...
docker cp "${ROOT_DIR}/procedures/hcg_node2vec_walk_v2.cpp" "${CONTAINER_NAME}:${REMOTE_DIR}/procedures/hcg_node2vec_walk_v2.cpp"
...
docker exec "${CONTAINER_NAME}" sh -lc "cd '${REMOTE_DIR}' && \
  g++ -std=c++17 -O2 -fPIC -shared -D_GLIBCXX_USE_CXX11_ABI=1 -Istub_include -I/usr/local/include \
    procedures/hcg_weighted_walk_v1.cpp -o out/hcg_weighted_walk_v1.so && \
  g++ -std=c++17 -O2 -fPIC -shared -D_GLIBCXX_USE_CXX11_ABI=1 -Istub_include -I/usr/local/include \
    procedures/hcg_node2vec_walk_v2.cpp -o out/hcg_node2vec_walk_v2.so && \
  ls -lh out/*.so"

docker cp "${CONTAINER_NAME}:${REMOTE_DIR}/out/hcg_weighted_walk_v1.so" "${OUT_DIR}/hcg_weighted_walk_v1.so"
docker cp "${CONTAINER_NAME}:${REMOTE_DIR}/out/hcg_node2vec_walk_v2.so" "${OUT_DIR}/hcg_node2vec_walk_v2.so"
