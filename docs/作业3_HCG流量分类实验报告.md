...
从特征组维度看，A 到 C 的提升路径清晰。A 组原始统计特征仅随机森林与 knn 勉强可用，其余分类器 Macro-F1 均未超过 0.3。B 组 HCG 嵌入将随机森林推至 0.414、knn 推至 0.294，证明图结构本身携带可观的分类信号。C 组融合两者后随机森林达到全部三组中的最高值 0.463，相较 A 组提升约 0.13，说明原始统计与图嵌入互补而非冗余，融合是一种有效的信号组织方式。决策树的轨迹同样佐证这一点：A 组 0.283、B 组 0.311、C 组 0.387，每加一层特征就上一个台阶。
...
> 待补截图 7：最优模型的混淆矩阵，展示类别间的混淆模式。
>
> 待补截图 8：各特征组 Macro-F1 对比柱状图。
