...
const char* kProcedureName = "hcg_node2vec_walk_v2";
...
struct GraphData {
    ...
    std::unordered_map<int64_t, std::unordered_set<int64_t> > neighbor_set;
    size_t node_count = 0;
    size_t edge_count = 0;
...
Params ParseParams(const std::string& request, std::vector<std::string>* warnings) {
    Params p;
    ...
    if (p.p <= 0.0 || !std::isfinite(p.p)) {
        p.p = 1.0;
        warnings->push_back("p <= 0 or non-finite; reset to 1.0.");
    }
    if (p.q <= 0.0 || !std::isfinite(p.q)) {
        p.q = 1.0;
        warnings->push_back("q <= 0 or non-finite; reset to 1.0.");
    }
...
GraphData LoadGraph(GraphDB& db, const Params& params, const std::vector<std::string>& initial_warnings) {
    ...
    for (auto vit = txn.GetVertexIterator(); vit.IsValid(); vit.Next()) {
        ...
        for (auto eit = vit.GetOutEdgeIterator(); eit.IsValid(); eit.Next()) {
            ...
            AddNeighbor(&data, src, dst, w);
            if (!params.directed) AddNeighbor(&data, dst, src, w);
            ++data.edge_count;
        }
    }
...
int64_t SampleFirstStep(const std::vector<Neighbor>& neighbors, std::mt19937_64* rng) {
...
bool HasEdgeFromPrevToCandidate(const GraphData& data, int64_t prev, int64_t candidate) {
    auto it = data.neighbor_set.find(prev);
    return it != data.neighbor_set.end() && it->second.find(candidate) != it->second.end();
}

int64_t SampleSecondOrder(const GraphData& data, const Params& params, int64_t prev, int64_t cur,
                          const std::vector<Neighbor>& neighbors, std::mt19937_64* rng) {
    if (neighbors.empty()) return -1;
    std::vector<double> cumulative;
    cumulative.reserve(neighbors.size());
    double total = 0.0;
    for (const auto& nb : neighbors) {
        // node2vec bias: p controls immediate return; q controls outward exploration.
        double alpha = 1.0;
        if (nb.vid == prev) {
            alpha = 1.0 / params.p;
        } else if (HasEdgeFromPrevToCandidate(data, prev, nb.vid)) {
            alpha = 1.0;
        } else {
            alpha = 1.0 / params.q;
        }
        total += nb.weight * alpha;
        ...
        for (int round = 0; round < params.num_walks; ++round) {
            for (int64_t start : data.start_nodes) {
                ...
                while (static_cast<int>(walk.size()) < params.walk_length) {
                    ...
                    int64_t next = -1;
                    if (walk.size() == 1) {
                        next = SampleFirstStep(it->second, &rng);
                    } else {
                        int64_t prev = walk[walk.size() - 2];
                        next = SampleSecondOrder(data, params, prev, cur, it->second, &rng);
                    }
                    if (next < 0) break;
                    walk.push_back(next);
                }
