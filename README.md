...
核心分类数据集 A/B/D/E/F 已上传到统一的 ModelScope 数据集仓库，可直接下载使用。数据集 C 可由 A + B 自动合成，无需单独下载。
...
### 7.2 从 HuggingFace 镜像下载

```bash
# 安装依赖
pip install huggingface_hub

# 如已同步 HuggingFace 镜像，可用同一脚本下载数据集（自动合成 C）
PYTHONPATH=src python3 scripts/download_datasets_from_hub.py \
  --hub huggingface \
  --repo-id MarkTom/IP-Network-Flow-Graph \
  --dataset-kind hcg
...
```bash
# 从 ModelScope 下载 D/E/F；如果 E/F 缺失，脚本仍可本地合成
PYTHONPATH=src python3 scripts/download_datasets_from_hub.py \
  --hub modelscope \
  --repo-id MarkTom/IP-Network-Flow-Graph \
  --dataset-kind tcg \
